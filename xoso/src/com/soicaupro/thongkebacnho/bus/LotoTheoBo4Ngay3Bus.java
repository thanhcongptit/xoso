...
public class LotoTheoBo4Ngay3Bus {

    private List<LotoTheoGiaiDTO> lotoTheoBos = new ArrayList<LotoTheoGiaiDTO>();

    public List<LotoTheoGiaiDTO> getListLotoBo4_3Ngay(String toDate, String dataDate, int miss) {
        ...
        try {
            ...
            AwardDetailDTO[] cacGiaiDate = new AwardDetailDTO[27];
            cacGiaiDate = getAwardDetailDTOInDate(dateAward, cacgiai);

            if (cacGiaiDate == null) {
                return lotoTheoBos;
            }

            int length = getLengthCacGiai(cacGiaiDate);
            if (length < 4) {
                return lotoTheoBos;
            }
            tr(1, length, 4, cacgiai, cacGiaiDate);

            System.out.println("--size: " + lotoTheoBos.size());
            for (int i = 0; i < lotoTheoBos.size(); i++) {
                System.out.println(lotoTheoBos.get(i).getValue() + "-" + lotoTheoBos.get(i).getTimes());
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
...
//		}
        ...
        if (k == 4) {
            ...
            AwardDetailDTO giai4 = cacgiaiDate[a[4] - 1];

            //System.out.println(giai1.getValue()+ "-"+ giai2.getValue() + "-" + giai3.getValue() + "-" + giai4.getValue());
            List<java.sql.Date> dates = getListDate(cacgiai, giai1.getValue(), giai2.getValue(),
                    giai3.getValue(), giai4.getValue(), giai1.getOpen_date());

            if (dates != null && dates.size() >= 20) {

                for (int t = 0; t < 50; t++) {
                    LotoTheoGiaiDTO lotoTheoGiaiDTO = getLotoGiaiDTO(cacgiai,
                            giai1.getValue(), giai2.getValue(), giai3.getValue(), giai4.getValue(),
                            dates, giai1.getOpen_date(), Constant.coupleLoto[t]);
    ...
    private LotoTheoGiaiDTO getLotoGiaiDTO(AwardDetailDTO[][] cacgiai,
            String giai1, String giai2, String giai3, String giai4,
            List<java.sql.Date> dates, java.sql.Date openDate, String loto[]) {
        ...
        lotoTheoGiaiDTO.setValue(giai1 + "," + giai2 + "," + giai3 + "," + giai4);
    ...
    private List<java.sql.Date> getListDate(AwardDetailDTO[][] cacgiai, String loto1, String loto2,
            String loto3, String loto4, java.sql.Date dateAward) {
        ...
        for (int i = 0; i < cacgiai.length; i++) {
            if (cacgiai[i][0].getOpen_date().before(dateAward)) {
                if (checkLotoInAward(cacgiai[i], loto1) && checkLotoInAward(cacgiai[i], loto2)
                        && checkLotoInAward(cacgiai[i], loto3) && checkLotoInAward(cacgiai[i], loto4)) {
    ...
    public static void main(String arg[]) {
        new LotoTheoBo4Ngay3Bus().getListLotoBo4_3Ngay("15-03-2016", "01-01-2005", 0);
