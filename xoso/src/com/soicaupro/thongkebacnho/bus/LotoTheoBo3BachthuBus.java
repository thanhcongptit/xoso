...
public class LotoTheoBo3BachthuBus {
	
	private List<LotoTheoGiaiBachThuDTO> lotoTheoBos = new ArrayList<LotoTheoGiaiBachThuDTO>();

	public List<LotoTheoGiaiBachThuDTO> getListLotoBo3BachThu(String fromDate, String toDate, String dataDate, int miss) {
...
//		}
			...
			if(dates != null && dates.size() >= 20) {
				...
				for (int t = 0; t < 100; t++) {
					...
					LotoTheoGiaiBachThuDTO lotoTheoGiaiDTO = getLotoGiaiDTO(cacgiai,
							giai1.getValue(), giai2.getValue(), giai3.getValue(), loto, dates, giai1.getOpen_date());
	...
	private LotoTheoGiaiBachThuDTO getLotoGiaiDTO(AwardDetailDTO[][] cacgiai, String giai1, String giai2, String giai3,
...
//			}
	...
	private List<java.sql.Date> getListDate(AwardDetailDTO[][] cacgiai, String loto1, String loto2,
			String loto3, java.sql.Date dateAward) {
		...
		for (int i = 0; i < cacgiai.length; i++) {
			if (cacgiai[i][0].getOpen_date().before(dateAward)) {
				if(checkLotoInAward(cacgiai[i], loto1) && checkLotoInAward(cacgiai[i], loto2) 
						&& checkLotoInAward(cacgiai[i], loto3)) {
	...
	public static void main(String arg[]) {
		//new LotoTheoBo3BachthuBus().getListLotoBo2BachThu("06-02-2016", "09-02-2016", "01-01-2015", 0);
