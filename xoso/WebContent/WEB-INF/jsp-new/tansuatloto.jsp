...
<html>
    ...
    <body style="background: #fff">
        <table width="100%" style="font-weight: bold;padding-left: 15px;padding-top: 10px;font-size: 15px;line-height:18px">
            <tr><td>Hướng dẫn sử dụng Thống Kê tần suất loto</td></tr>
            <tr>
                <td >
            ...
            <tr>
                <td>
                    ...
                    <c:if test="${capso==null}">
                        <c:forEach begin="0" end="99" var="i" varStatus="status">                        
                            <c:if test="${i<10}">
                                <input type="checkbox" name="capso" value="0${i}" id="capso"/>
                                <span style="color: red">0${i}</span>
                    ...
                    <c:if test="${capso!=null}">
                        <c:forEach begin="0" end="99" var="i" varStatus="status">
                            ...
                            <c:if test="${i<10}"><c:set var="ii" value="0${i}"/></c:if>
                            <c:forEach items="${capso}" var="cs">
                                <c:if test="${cs==ii}"><c:set var="chk" value="checked"/></c:if>
                            </c:forEach> 
                                <c:if test="${i<10}">                                    
                                    <input type="checkbox" name="capso" value="0${i}" id="capso" ${chk}/>
                                    <span style="color: red">0${i}</span>
                                    <c:if test="${status.count%10==0}"><br/></c:if>
                                </c:if>
                                <c:if test="${i>=10}">
                                ...
                                <td>
                                    <table  width="100%" class="tansuatloto_table" cellspacing="0" cellpadding="0">
                                        <c:forEach items="${tansuats}" var="tansuat" varStatus="status">
                                            ...
                                            <c:if test="${status.index==0}">
                                                <tr style="height: 30px">
                                                <td width="${rate}%" class="tansuatloto_td" >${fn:replace(fn:substring(tansuat.openDate, 0, 5),'/', '<br/>-<br/>')}</td>
                                                </tr>
                                            </c:if>                                    
                                            <tr style="height: 30px">    
                                            <c:if test="${tansuat.solanve>0}"><td width="${rate}%" class="tansuatloto_td" <c:if test="${tansuat.special}">style="color: red"</c:if>>${tansuat.solanve}</td></c:if>
