     ...
     <%
         ...
         if(!CommonUtil.isEmptyString(datetime)) {
        	 results = new LotoTheoGiaiBus().getListLotoTheoGiaiDTOs
        			 (date_begin, date_end, datetime, Integer.parseInt(miss), 3);
         }
        ...
        <div id="tkv-wrapper" class="fix-clear"><!-- Bắt đầu phần thân -->
            ...
            <section class="sc-section">
                <div class="tkv-content">
                    <h1 class="sc-tt">(Soi cầu PRO, Lotto cặp về trong 3 ngày theo giải)</h1>
                </div>
            </section>

            <section class="sc-section"> 
                <div class="sc-grp-columb input-prepend">
                    <form method="POST" action="${pageContext.servletContext.contextPath}/theogiai_rs_loto_3_ngay.htm" accept-charset="UTF-8" class="frm" name="frm" autocomplete="off">
                        		...
                        		<tr>
                        			...
                        			<% for(LotoTheoGiaiDetailDTO detailDTO: detailDTOs) {%>
                        				<td style="<% if(detailDTO.getColorDay().equals("1")) {%> background-color:#FFCC66; <%}
                        					else if(detailDTO.getColorDay().equals("2")) {%>background-color:#99CC00 <% } else if(detailDTO.getColorDay().equals("3")) {%>background-color:#99FF66 <% }%>"><%=detailDTO.getRs() %></td>
                        			<%} %>
